...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="{{ route('dosenHistory') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
               <svg class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 20">
                  <path d="M16 14V2a2 2 0 0 0-2-2H2a2 2 0 0 0-2 2v15a3 3 0 0 0 3 3h12a1 1 0 0 0 0-2h-1v-2a2 2 0 0 0 2-2ZM4 2h2v12H4V2Zm8 16H3a1 1 0 0 1 0-2h9v2Z"/>
              </svg>
               <span class="flex-1 ml-3 whitespace-nowrap">History</span>
        ...
        <li>
            ...
            <ul id="dropdown-example" class="hidden py-2 space-y-2">
                  <li>
                     <a href="{{ route('dosen.pen05Home') }}" class="flex text-sm items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Seminar Proposal (PEN 05)</a>
                  </li>
                  <li>
                     <a href="{{ route('dosen.pen09Home') }}" class="flex text-sm items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Seminar Hasil (PEN 09)</a>
                  </li>
                  <li>
                     <a href="{{ route('dosen.pen14Home') }}" class="flex text-sm items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Sidang Akhir (PEN 14)</a>
                  </li>
            </ul>
         </li>
        @endif
         ...
         <li>
            ...
            <ul id="dropdown-example" class="hidden py-2 space-y-2">
                  @php
                    $userId = auth()->user()->id;
                    $seminarProposalRegistered = \App\Models\Sempro::where('id_mahasiswa', $userId)
                        ->where('seminar', 'Seminar Proposal')
                        ->exists();
                    
                    $seminarHasilRegistered = \App\Models\Sempro::where('id_mahasiswa', $userId)
                        ->where('seminar', 'Seminar Hasil')
                        ->exists();
                    
                    $seminarSidangRegistered = \App\Models\Sempro::where('id_mahasiswa', $userId)
                        ->where('seminar', 'Sidang Akhir')
                        ->exists();
                  @endphp
                <li>
                    @if ($seminarProposalRegistered)
                    <a data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" type="button">
                      Proposal
                    </a>
                    @php
                      $pesansempro = 'Anda Sudah Seminar Proposal';
                    @endphp
                    @else
                    <a href="/seminar/create" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">
                        Proposal
                    </a>
                    @endif
                </li>
                  ...
                  <li>
                    ...
                    @php
                      $pesansemhas = 'Anda Sudah Seminar Hasil';
                    @endphp
                    @elseif (!$seminarProposalRegistered)
                    <a data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" type="button">
                    Hasil
                    </a>
                    @php
                      $pesanbelumsempro = 'Anda Belum Seminar Proposal';
                    @endphp
                    @else
                  ...
                  <li>
                  @if ($seminarSidangRegistered)
                    <a data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" type="button">
                    Sidang
                    </a>
                    @php
                      $pesansidang = 'Anda Sudah Sidang Akhir';
                    @endphp
                    @elseif (!$seminarHasilRegistered)
                    <a data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" type="button">
                    Sidang
                    </a>
                    @php
                      $pesanbelumsemhas = 'Anda Belum Seminar Hasil';
                    @endphp
                    @else
    ...
    <div class="relative w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 text-center">
                ...
                @elseif (!$seminarHasilRegistered)
                    {{$pesanbelumsemhas}}
                @elseif ($seminarSidangRegistered)
                    {{$pesansidang}}
                @endif
                </h3>
